...
def sort_campeones_por_nombre():
    ...
    return sorted(champions_in_csv(), reverse=False, key=lambda x: x[0])
...
def sort_por_precio_rp():
    ...
    return sorted(champions_in_csv(), reverse=False, key=lambda x: int(x[7]))
...
def sort_por_gamestyle():
    ...
    return sorted(champions_in_csv(), reverse=False, key=lambda x: x[3])
