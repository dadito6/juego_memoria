...
def build():
    ...
    path_buttons = os.path.join(os.getcwd(), 'src', 'Entities', 'Graphics', 'Menu', 'Buttons', sg.theme())
    title_file = sg.theme().split()[1].lower() + '_title.png'
    pad_buttons = (116, 116)

    layout = [
        [sg.Image(filename=os.path.join(os.getcwd(), 'src', 'Entities', 'Graphics', 'Menu', 'Background', title_file),
